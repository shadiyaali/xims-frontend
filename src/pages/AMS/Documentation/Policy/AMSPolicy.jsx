...
const AMSPolicy = () => {
    ...
    const fetchPolicies = async () => {
      try {
        ...
        const response = await axios.get(`${BASE_URL}/company/policy-ams/${companyId}/`);
    ...
    const handleDeletePolicy = async (policyId) => {
      if (window.confirm("Are you sure you want to delete this policy?")) {
        try {
          await axios.delete(`${BASE_URL}/company/policy-ams/${policyId}/delete/`);
      ...
      <div className="bg-[#1C1C24] rounded-lg text-white p-5">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          {qmsPolicies.length === 0 && (
            <button
              className="bg-transparent border border-[#DD6B06] text-[#DD6B06] rounded-[4px] p-[10px] flex items-center justify-center gap-[10px] transition-all duration-200 w-[140px] h-[42px] add-policy-btn hover:bg-[#DD6B06] hover:text-white group"
                ...
                <div key={policy.id} className="px-5 pt-6 pb-5 border-b border-gray-700 flex justify-start space-x-[50px] items-center last:border-b-0">
                  <div className="flex items-center gap-[50px]">
                    <div className='gap-[15px] flex flex-col max-w-xl'>
                      <span className="policy-name text-[#DD6B06] truncate">
                        Asset Policy
                      </span>
